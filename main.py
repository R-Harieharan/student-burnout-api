...
def predict_performance(data: StudentDataInput):
    ...
    try:
        ...
        if hasattr(model, "predict_proba"):
            ...
            # Assuming 0 is 'Not High' and 1 is 'High' based on y_train_binary mapping
            not_high_prob = float(probabilities[0])
            high_prob = float(probabilities[1])
            predicted_class_confidence = float(probabilities[prediction_int])
        else:
            # Fallback if probability isn't supported
            predicted_class_confidence = 1.0
            if prediction_int == 1: # High
                high_prob = 1.0
            else: # Not High
                not_high_prob = 1.0
        ...
        return {
            ...
            "confidence": round(predicted_class_confidence, 4),
